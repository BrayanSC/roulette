$(function(){
	...
	$('.stop').click(function(){
		var stopImageNumber2 = -1;
		if(stopImageNumber2 == "") {
			stopImageNumber2 = null;
		}
	...
	var updateStopImageNumber2 = function(stopImageNumber2) {
		$('.image_sample').children().css('opacity' , 0.2);
		$('.image_sample').children().filter('[data-value="' + stopImageNumber2 + '"]').css('opacity' , 1);
		$('.stop_image_number_param').text(stopImageNumber2);
		updateParamater();
	}
	...
	$('#stopImageNumber').spinner({
		spin: function( event, ui ) {
			...
			updateStopImageNumber2(imageNumber2);	
		}
	});
